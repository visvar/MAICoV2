...
// 1 distant
export function newSimilarityTestFunction(rec1, rec2, index1, index2) {
  if (rec1 === undefined)
    return 1
  if (rec2 === undefined)
    rec2 = allPrimer[0]
  ...
  rec1.notes.forEach((note, j) => {
    if (j === 0)
      ...
      line1.push(
        {
          x:
            note.quantizedStartStep + 0.25,
          y:
            note.pitch
        }
      );
    if (j === rec1.notes.length - 1)
      line1.push(
        {
          x:
      ...
      line1.push(
        {
          x:
            note.quantizedEndStep - 0.25,
          y:
            note.pitch
        }
      );
  });
  rec2.notes.forEach((note, j) => {
    if (j === 0)
      line2.push({
        x:
          note.quantizedStartStep,
  ...
  console.log(index1, index2, sim10, sim30, sim50, sim100)

  return (sim10 + sim30 + sim50 + sim100) / 4
...
import { curveLength, shapeSimilarity } from 'curve-matcher';
