...
export function PianorollHeatmap(data, svg, h, w, xDomain, yDomain, maxocc) {

    const margin = { top: 20, right: 10, bottom: 10, left: 25 };

    if (svg !== undefined && svg !== null && data !== null) {
...
export function PianorollHeatmapFilter(data, svg, h, w, xDomain, yDomain, maxocc) {
    ...
    if (svg !== undefined && svg !== null && data !== null) {
        ...
        let datalow = data.map((p,i) => [i,p[0]])
        let datahigh = data.map((p,i) => [i,p[1]])

        let line = d3.line().x((d)=>x(d[0])).y((d)=>y(d[1])).curve(d3.curveMonotoneX)


        svg.append('g')
            .append('path')
            .attr("class","filterlinelow")
            .style('stroke', 'blue')
            .style("fill", "none")
            .style('opacity', 0.3)
            .attr('d', line(datalow))
        ...
        svg.append('g')
            .append('path')
            .attr("class","filterlinehigh")
            .style('stroke', 'red')
            .style("fill", "none")
            .style('opacity', 0.3)
            .attr('d', line(datahigh)
            )
