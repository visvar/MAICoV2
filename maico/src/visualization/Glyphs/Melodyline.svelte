<script>
    ...
    import { modelColor10 } from "../../util/visutil.js";
    ...
    $: render = ({ context }) => {
        ...
        for(let i = 0; i<=voices;i++){
            ...
            let notes = information.isPolymix?information.melody.notes.filter(n => n.meloID === i):information.melody.notes
            const line = []
            ...
            context.stroke();
            notes.forEach((note,j) => {
                mvlib.Canvas.drawRoundedRect(context, scaleX(note.quantizedStartStep), y - r - 2 + (height * i), scaleX(note.quantizedEndStep)-scaleX(note.quantizedStartStep), height, 0);
                context.fill()
                context.stroke()
            })
        }
